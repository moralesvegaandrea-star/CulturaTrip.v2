...
def parse_comunidades_sheet(raw_sheet: pd.DataFrame, sheet_name: str) -> pd.DataFrame:
    """
    ...
    df["dc"] = df["dc"].astype(str).str.zfill(3)
...
for sheet_name, raw_sheet in sheets.items():
    ...
    df_sheet = parse_comunidades_sheet(raw_sheet, sheet_name)
    dfs.append(df_sheet)

df_codmun = pd.concat(dfs, ignore_index=True)
# Crear ID único nacional
df_codmun["id_municipio"] = df_codmun["cpro"] + df_codmun["cmun"]
print(df_codmun.head())
print("Ciudades unificadas:", df_codmun.shape)
print(df_codmun.head(10))
print("Provincias:", df_codmun["provincia"].unique())
print(df_codmun.tail(10))

#Importar nuevo dataset a cvs
os.makedirs(RAW_DIR, exist_ok=True)
output_path = os.path.join(RAW_DIR, "df_codmun.csv")
df_codmun.to_csv(
    output_path,
    index=False,
    encoding="utf-8-sig"
